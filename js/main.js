...
function FormValidation() {
    var isValid = true;
    isValid &= KiemTraRong('ho', 'thongBaoHo', 'Vui lòng nhập họ!') && KiemTraDinhDangChu('ho', 'thongBaoHo', 'Họ phải là chữ!');
    isValid &= KiemTraRong('ten', 'thongBaoTen', 'Vui lòng nhập tên!') && KiemTraDinhDangChu('ten', 'thongBaoTen', 'Tên phải là chữ!');
    isValid &= KiemTraMa(2, 6);
...
function themNhanVien() {
    if (FormValidation()) {
        ...
        // Thêm nhan viên len api
        nhanvienSV.themDSNV(nhanVienMoi).then(function (res) {
            console.log(res)
...
function xoaNhanVien(index) {
    Swal({
        ...
        if (result.value) {
            nhanvienSV.xoaDSNV(DSNV[index]).then(function (res) {
                console.log(res)
                DSNV.splice(index, 1);
                taoBang(DSNV);
            })
...
function CapNhatNhanVien() {
    ...
    // Cập nhật nhân viên thông qua api
    nhanvienSV.capnhatDSNV(nhanVienCapNhat).then(function (res) {
        console.log(res);
        //Lấy vị trí của nhân viên cần thay thế
        var index = findIndex(DSNV, msnv);
        //Thay thế nhân viên cũ bằng nhân viên mới cập nhật
